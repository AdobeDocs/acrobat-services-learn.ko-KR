...
그런 다음 `downloadContract` 클래스 내에서 다음과 같이 `PersonController` 메서드를 구현합니다.
...
그런 다음 `AdobeSignController`을(를) 구현하는 다른 컨트롤러 `sendContractMethod`을(를) 추가합니다(동반 코드 참조). 이 방법은 다음과 같이 작동합니다.
